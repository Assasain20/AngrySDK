...
function setBlockGroupIndices()
  ...
  for blockName, block in _G.pairs(blockTable.blocks) do
    if block.group == nil then
      ...
      local index
      if blockTable.groups[block.group] == nil then --Mensaje de error
...
function updateEditor(dt, time)  
  ...
  if keyPressed["LBUTTON"] then
    ...
    if not keyHold["SPACE"] then
      ...
      elseif objectToAdd ~= nil and not editor.previewSprite then
        if not editor.drawOneLayer then
		
			if isGridSnapOn == false then
				xPosition, yPosition = cursorPhysics.x, cursorPhysics.y
			else
				xPosition = cursorPhysics.x - (cursorPhysics.x % 0.5)
				yPosition = cursorPhysics.y - (cursorPhysics.y % 0.5)
				if (cursorPhysics.x % 0.5) >= 0.25 then
					xPosition = xPosition + 0.5
				end
				if (cursorPhysics.y % 0.5) >= 0.25 then
					yPosition = yPosition + 0.5
				end		
			end		  
		  local name = createObject(blockTable, objectToAdd, nil, xPosition, yPosition)
      ...
      if copiedObjects ~= nil then
        ...
        for k, v in _G.pairs(copiedObjects) do
          if editor.drawOneLayer then
            ...
            if isGridSnapOn == false then
				xPosition, yPosition = cursorPhysics.x + v.x, cursorPhysics.y + v.y
			else
				xPosition = (cursorPhysics.x - (cursorPhysics.x % 0.5) + v.x) - ((cursorPhysics.x - (cursorPhysics.x % 0.5) + v.x)%0.5)
				yPosition = (cursorPhysics.y - (cursorPhysics.y % 0.5) + v.y) - ((cursorPhysics.y - (cursorPhysics.y % 0.5) + v.y)%0.5)
				if ((cursorPhysics.x - (cursorPhysics.x % 0.5) + v.x)%0.5) >= 0.25 then
					xPosition = xPosition + 0.5
				end
				if ((cursorPhysics.y - (cursorPhysics.y % 0.5) + v.y)%0.5) >= 0.25 then
					yPosition = yPosition + 0.5
				end
			end					
            local name = createObject(blockTable, v.definition, nil, xPosition, yPosition)
  ...
  if keyHold["SHIFT"] and keyPressed["S"] then
    -- Toggles the grid snap (it's disabled by default)
	isGridSnapOn = not isGridSnapOn
  end  
  ...
  if editor.spriteBar ~= nil and editor.spriteBar.getBounds and not keyHold.LBUTTON then
    local spriteBarWidth, spriteBarHeight = editor.spriteBar:getBounds()
    local x = cursor.x - editor.spriteBar.x
    local y = cursor.y - editor.spriteBar.y + editor.spriteBar.button_height
    if x > 0 and spriteBarWidth > x and y > 0 and spriteBarHeight > y then
      editor.spriteBar:onPointerEvent("HOVER", x, y)
    end
  end
  defaultCamera(dt)
  oldCursor.x = cursor.x
...
function drawEditor()  
    ...
    drawGame()
      for k, v in _G.pairs(objects.world) do
        if v.startNumber then
          ...
          setRenderState(-screen.left / blockDefScale, -screen.top / blockDefScale, worldScale, worldScale, 0, pivotX, pivotY)
          local x, y = physicsToWorldTransform(v.x, (v.y+v.radius)-5.5)
          _G.res.drawString("TEXTS_BASIC", "" .. v.startNumber, x, y, "HCENTER", "VCENTER")
        end
		if v.specialBlock then 
          local sprite = blockTable.blocks[v.definition].sprite
		  local blockDefScale = blockTable.blocks[v.definition].scale or 1
          local pivotX, pivotY = _G.res.getSpritePivot(sprite)
          setRenderState(-screen.left / blockDefScale, -screen.top / blockDefScale, worldScale, worldScale, 0, pivotX, pivotY)
          local x, y = physicsToWorldTransform(v.x, v.y)
		  if getObjectDefinition(k).audioInfo then
			_G.res.drawSprite("", "BUTTON_ICON_SFX_1", x, y)
		  elseif getObjectDefinition(k).animationSprites then
		    _G.res.drawSprite("", "BUTTON_ICON_RESUME_1", x, y)
		  end
		end
      end
      setRenderState(0, 0, 1, 1, 0)
  ...
  setRenderState(0, 0, 1, 1, 0)
  _G.res.drawString("TEXTS_BASIC", levelName--[[.." \''"]], screenWidth * 0.5, 0, "TOP", "HCENTER")
  ...
  if selectedObjects[1] ~= nil then
    _G.res.drawString("TEXTS_BASIC", "Name: "..selectedObjects[1].name, 0, 105, "TOP", "LEFT")
	_G.res.drawString("TEXTS_BASIC", "X: "..selectedObjects[1].x.." - Y: "..selectedObjects[1].y, 0, 140, "TOP", "LEFT")
  ...
  if objects.worldGravity then
    _G.res.drawString("", "Gravity: " .. objects.worldGravity, screenWidth * 0.5, 70, "TOP", "HCENTER")
  end
  if isGridSnapOn == false then
		_G.res.drawString("", "Grid Snap off", screenWidth, screenHeight-36, "BOTTOM", "RIGHT")
	elseif isGridSnapOn == true then
		_G.res.drawString("", "Grid Snap on", screenWidth, screenHeight-36, "BOTTOM", "RIGHT")
  end
  if blockTable.themes[objects.theme].friction then
	_G.res.drawString("", "Static block friction: " .. (blockTable.themes[objects.theme].friction or "error??"), 0, screenHeight-36, "TOP", "LEFT")
  end
  _G.res.drawString("", "Theme index: " .. (currentThemeIndex or "error??"), screenWidth * 0.5, 35, "TOP", "HCENTER")
  setRenderState(0, 0, 1, 1, 0)
...
function refreshGroundTextures()
  ...
  for k, v in _G.pairs(objects.world) do
    ...
    if bDef.group == "staticBlocks" then		
		setTexture(v.name, blockTable.themes[currentTheme].texture or "INGAME_THEME_MISSING_TEXTURE")
		-- This doesn't work yet -Assasain_2.0
		if blockTable.themes[objects.theme].friction then		
			bDef.friction = blockTable.themes[objects.theme].friction
		end
