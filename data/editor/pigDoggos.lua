...
function updateEditor(dt, time)
  ...
  if keyReleased["ESCAPE"] or touchcount == 3 then
    ...
    local prompt = {
      ...
      confirm_call = okFunction
    }
    notificationsFrame:addChild(ui.ConfirmPrompt:new(prompt))
    return
  end
  ...
  if keyHold["SHIFT"] and keyPressed["Y"] then
    currentThemeIndex = currentThemeIndex - 1
    if currentThemeIndex > blockTable.themes.settings.themeAmount then
      currentThemeIndex = 16
    end
...
function drawEditor()
  ...
  if selectedObjects[1] ~= nil then
    _G.res.drawString("", "X: " .. selectedObjects[1].x, 0, 140, "TOP", "LEFT")
    _G.res.drawString("", "Y: " .. selectedObjects[1].y, 0, 175, "TOP", "LEFT")
  end  
...
function refreshGroundTextures()
  ...
  for k, v in _G.pairs(objects.world) do
    ...
    if bDef.isBackgroundMask then
      setTexture(v.name, themeBgTexture or themeTexture)
    elseif bDef.group == "fg" then
      setTexture(v.name, themeTexture)
    end
  end
end
selectedObjectsAreaCoords = nil
